...
@interface FcsAddressPickerView ()<UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate>
{
    ...
    NSString       * _addressString, * _addressId;
    NSInteger        _selfSelectCount; //第几级菜单
    
    FcsAddressButton     * _provinceButton,  * _cityButton, *_areaButton;
...
@implementation FcsAddressPickerView
/** 初始化 self */
...
-(void)showWithProvinceName:(NSString *)provinceName{
    ...
    _provinceArray = [FcsAddressManager getLocalAreaArray];
    
    [_provinceArray enumerateObjectsUsingBlock:^(NSDictionary *provinceDic, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)configUI {
    CGFloat top = 40;
    //标题
    UILabel * titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 10, ScreenWidth, 20)];
    titleLabel.text = @"所在地区";
    titleLabel.font = [UIFont systemFontOfSize:14];
    titleLabel.textAlignment = NSTextAlignmentCenter;
    titleLabel.textColor = C333333;
    ...
    [_selectButton setTitleColor:C759A15 forState:UIControlStateNormal];
    ...
    //选中 线
    _selectLineView = [[UIView alloc] initWithFrame:CGRectMake(19, _selectScrollViewHeight - 1 , SelectLineWidth, 1)];
    _selectLineView.backgroundColor = C759A15;
    ...
    [_provinceTableView registerClass:[FcsAddressTableCell class] forCellReuseIdentifier:@"cell_province"];
    ...
    [_cityTableView registerClass:[FcsAddressTableCell class] forCellReuseIdentifier:@"cell_city"];
    [_addressScrollView addSubview:_cityTableView];
    
    //地区 列表
    _areaTableView = [self configTableDefaultWithHeight:_addressHeight andIndex:3];
    [_areaTableView registerClass:[FcsAddressTableCell class] forCellReuseIdentifier:@"cell_area"];
...
- (UITableView *)configTableDefaultWithHeight:(CGFloat)height andIndex:(NSInteger)index {
    UITableView * table = [[UITableView alloc] initWithFrame:CGRectMake(ScreenWidth * (index - 1), 0, ScreenWidth * index,height - 65) style:UITableViewStylePlain];
    table.delegate = self;
...
- (void)configAddressClear {
    ...
    _selectButton.frame = CGRectMake(21, SelectLabelTop, 100, SelectLabelHeight);
    _selectLineView.frame = CGRectMake(21, _selectScrollViewHeight - 1 , SelectLineWidth, 1);
    ...
    for(NSInteger i = 0; i < btnsCount; i ++) {
        FcsAddressButton * btn = _buttonsArray[0];
    ...
    if(scrollView == _selectScrollView) {
        ...
        if(lroundf(page_point * 1000) == 0) {
            ...
            if(_buttonsArray.count > pageNum) {
                FcsAddressButton * btn = (FcsAddressButton *)_buttonsArray[pageNum];
                lineLeft = btn.left;
                lineWidth = btn.width;
            ...
            [UIView animateWithDuration:ViewAnimal animations:^{
                _selectLineView.frame = CGRectMake(lineLeft, _selectScrollViewHeight - 1, lineWidth, 1);
...
- (void)setTableViewCellOfCentenWith:(NSInteger)pageNum is_delayed:(BOOL)is_delayed{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(is_delayed * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if(pageNum == 0) {
            ...
            [_provinceTableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0] atScrollPosition:UITableViewScrollPositionMiddle animated:YES];
        }
        else if(pageNum == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger pro_row = _firstCurrentIndex - 1;
    ...
    if(tableView == _provinceTableView) {
        ...
        titleString = [[_provinceArray objectAtIndex:pro_row] objectForKey:@"name"];
    ...
    FcsAddressTableCell * cell = [tableView dequeueReusableCellWithIdentifier:cellIdentifier];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(tableView == _provinceTableView) {
        _selfSelectCount = 1;
        _provinceLine = pro_row;
        _selfNotFirstShow = NO;
        
        name = [[_provinceArray objectAtIndex:pro_row] objectForKey:@"name"];
        
        //城市数组
        _cityArray = _provinceArray[_provinceLine][@"child"];
        ...
        //区的
        _areaArray = _cityArray[_cityLine][@"child"];
...
- (void)createButtonsWithTitle:(NSString *)title andIndex:(NSInteger)index {
    ...
    if(_buttonsArray.count > index) {
        for(NSInteger i = index; i <= _buttonsArray.count; i++) {
            FcsAddressButton * btn = _buttonsArray[index];
        ...
        FcsAddressButton * btn = _buttonsArray[index - 1];
        ...
        //_buttonsLasterLeft
        FcsAddressButton * btn = [FcsAddressButton buttonWithType:UIButtonTypeCustom];
...
- (void)addressTitleButtonClick:(FcsAddressButton *)button {
    [self scrollViewChangeAnimal:button.tagFcs andLineWidth:button.width   andLineLeft:button.left];
    
    [_addressScrollView setContentOffset:CGPointMake((button.tagFcs - 1) * ScreenWidth, 0) animated:YES];
}
...
- (void)completionAddress {
    ...
    NSString * provinceId = _provinceArray[_provinceLine][@"id"];
    NSLog(@"省份id == %@",provinceId);
    NSString * cityId = _cityArray[_cityLine][@"id"];
    NSString * areaId = _areaArray[_areaLine][@"id"];
    ...
    self.completion(_addressString, _addressId);
    [self hiddenAddress];
}
